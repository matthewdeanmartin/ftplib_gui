# Security Policy for `ftplib_gui`
...
For maximum isolation, run `ftplib_gui` inside a container so it only has access to what you
explicitly mount.
...
RUN pip install --no-cache-dir ftplib_gui

ENTRYPOINT ["ftplib_gui"]
CMD ["--help"]
```
...
```bash
docker build -t ftplib_gui .

# Mount the current directory into /data and pass it as an argument
docker run --rm -v "$(pwd):/data" ftplib_gui /data
```
