...
PACKAGE := ftplib_gui
PYTHON_TARGETS := ftplib_gui tests
PYLINT_MAIN_TARGETS := ftplib_gui
...
ABOUT_FILE := ftplib_gui/__about__.py
...
dont-be-lazy:
	...
	@$(UV) dont_be_lazy --root . --no-color scan ftplib_gui --no-config-suppressions || true

.PHONY: pydoc-docs
pydoc-docs:
	@$(UV) pydoc_fork ftplib_gui -o ./pydoc/
